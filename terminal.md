...
`cat`: is used to print the current working directory to know where we are in the folder structure
...
`git commit -m "desc"`: is used to **commit** the files, and save their history. You can only use this command if you have staged the files using the `git add` command.
